...
<script type="text/javascript" src="__TMPL__Static/Js/role.js"></script>
...
<table cellpadding="4" cellspacing="0" border="0" class="table-form">
	<tr>
		<th width="230" style="border-right:solid 1px #CCC;"><?php echo L("NAME");?></th>
		<td><input type="text" class="textinput requireinput" name="name" /></td>
	</tr>
	<tr>
		<th style="border-right:solid 1px #CCC;"><?php echo L("ACCESS");?></th>
		<td><p><?php echo L("ACCESS_TIPS");?></p></td>
	</tr>
	<tr>
		<td class="first tc" style="border-right:solid 1px #CCC;"><p><?php echo L("MODULE_NAME");?></p></td>
		<td class="tc"><p><?php echo L("ACTION_NAME");?></p></td>
	</tr>
	<?php if(is_array($access_list)): foreach($access_list as $key=>$module): ?><tr class="access-item">
		<td class="first tr" style="border-right:solid 1px #CCC;">
			<p><label><input class="module-item" type="checkbox" name="access_node[]" value="<?php echo ($module["id"]); ?>" /><span>&nbsp;<?php echo ($module["module_name"]); ?>&nbsp;</span></label></p>
		</td>
		<td>
			<p>
				<?php if(isset($module['actions'])): ?><label><input class="select-all" type="checkbox" />&nbsp;<span><?php echo L("SELECT_ALL");?></span></label>&nbsp;<?php endif; ?>
				<?php if(is_array($module["actions"])): foreach($module["actions"] as $key=>$action): ?><label><input class="action-item" type="checkbox" name="access_node[]" value="<?php echo ($action["id"]); ?>" /><span>&nbsp;<?php echo ($action["action_name"]); ?></span></label>&nbsp;&nbsp;<?php endforeach; endif; ?>&nbsp;
			</p>
		</td>
