...
<div class="handle-btns">
	<div class="img-button "><p><input type="button" id="addRole" name="addRole" value="<?php echo L("ADD");?>" onclick="addData()" class="addRole"></p></div>
	<div class="img-button "><p><input type="button" id="editRole" name="editRole" value="<?php echo L("EDIT");?>" onclick="editData(this,'checkList')" class="editRole"></p></div>
	<div class="img-button "><p><input type="button" id="removeRole" name="removeRole" value="<?php echo L("REMOVE");?>" onclick="removeData(this,'checkList')" class="removeRole"></p></div>
</div>
<!-- Think 系统列表组件开始 -->
<table id="checkList" class="table-list list" cellpadding="0" cellspacing="0" border="0"><thead><tr><th width="30" class="first"><input type="checkbox" onclick="checkAll('checkList')"></th><th width="50" ><a href="javascript:sortBy('id','<?php echo ($sort); ?>','index')" title="按照<?php echo L("ID");?><?php echo ($sortType); ?> "><?php echo L("ID");?><?php if(($order)  ==  "id"): ?><img src="__TMPL__Static/Images/<?php echo ($sortImg); ?>.gif" align="absmiddle"><?php endif; ?></a></th><th ><a href="javascript:sortBy('name','<?php echo ($sort); ?>','index')" title="按照<?php echo L("NAME");?><?php echo ($sortType); ?> "><?php echo L("NAME");?><?php if(($order)  ==  "name"): ?><img src="__TMPL__Static/Images/<?php echo ($sortImg); ?>.gif" align="absmiddle"><?php endif; ?></a></th><th width="50" ><a href="javascript:sortBy('status','<?php echo ($sort); ?>','index')" title="按照<?php echo L("STATUS");?><?php echo ($sortType); ?> "><?php echo L("STATUS");?><?php if(($order)  ==  "status"): ?><img src="__TMPL__Static/Images/<?php echo ($sortImg); ?>.gif" align="absmiddle"><?php endif; ?></a></th><th width="100">操作</th></tr></thead><tbody><?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): ++$i;$mod = ($i % 2 )?><tr ="<?php echo ($node[""]); ?>" class="<?php if(($mod)  ==  "0"): ?>even<?php endif; ?>"><td class="first"><input type="checkbox" name="key"	value="<?php echo ($node["id"]); ?>"></td><td ><?php echo ($node["id"]); ?></td><td align="left" ><span class="pointer" module="Role" href="javascript:;" onclick="textEdit(this,'<?php echo (addslashes($node["id"])); ?>','name')"><?php echo ($node["name"]); ?></span></td><td ><span class="pointer" module="Role" href="javascript:;" onclick="toggleStatus(this,'<?php echo (addslashes($node["id"])); ?>','status')"><?php echo (getStatusImg($node["status"])); ?></span></td><td><a href="javascript:;" onclick="editData(this,'<?php echo ($node["id"]); ?>','id')"><?php echo L("EDIT");?></a>&nbsp;&nbsp;<a href="javascript:;" onclick="removeData(this,'<?php echo ($node["id"]); ?>','id')"><?php echo L("REMOVE");?></a>&nbsp;&nbsp;</td></tr><?php endforeach; endif; else: echo "" ;endif; ?></tbody></table>
<!-- Think 系统列表组件结束 -->
